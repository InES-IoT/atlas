...
mod tests {
    ...
    fn add_lib_not_found() {
        let mut at = Atlas::new(&*NM_PATH, file!()).unwrap();
        let err = at
            .add_lib(SymbolLang::Rust, "lksjdflkjsdflkjsdf")
            .unwrap_err();
        assert_eq!(err.kind(), ErrorKind::Io);
    }
    ...
    fn report_without_analyze() {
        ...
        assert!(at
            .report_syms(vec![SymbolLang::Rust], MemoryRegion::Rom, None)
            .is_none());
    }
    ...
    fn report_lang_c_no_lib() {
        ...
        assert_eq!(
            lang_rep.size(SymbolLang::C, MemoryRegion::Both).as_u64(),
            2154
        );
        assert_eq!(
            lang_rep.size(SymbolLang::Cpp, MemoryRegion::Both).as_u64(),
            0
        );
        assert_eq!(
            lang_rep.size(SymbolLang::Rust, MemoryRegion::Both).as_u64(),
            0
        );

        assert_eq!(
            lang_rep.size(SymbolLang::C, MemoryRegion::Rom).as_u64(),
            842
        );
        assert_eq!(
            lang_rep.size(SymbolLang::Cpp, MemoryRegion::Rom).as_u64(),
            0
        );
        assert_eq!(
            lang_rep.size(SymbolLang::Rust, MemoryRegion::Rom).as_u64(),
            0
        );

        assert_eq!(
            lang_rep.size(SymbolLang::C, MemoryRegion::Ram).as_u64(),
            1312
        );
        assert_eq!(
            lang_rep.size(SymbolLang::Cpp, MemoryRegion::Ram).as_u64(),
            0
        );
        assert_eq!(
            lang_rep.size(SymbolLang::Rust, MemoryRegion::Ram).as_u64(),
            0
        );
    ...
    fn report_lang_c_app_rust_lib() {
        ...
        assert!((lang_rep.size_pct(SymbolLang::Cpp, MemoryRegion::Both) - 0_f64).abs() < 1e-8);
        assert!(
            (lang_rep.size_pct(SymbolLang::Rust, MemoryRegion::Both) - 59.75705329).abs() < 1e-8
        );

        assert!((lang_rep.size_pct(SymbolLang::C, MemoryRegion::Rom) - 19.69214437).abs() < 1e-8);
        assert!((lang_rep.size_pct(SymbolLang::Cpp, MemoryRegion::Rom) - 0_f64).abs() < 1e-8);
        assert!(
            (lang_rep.size_pct(SymbolLang::Rust, MemoryRegion::Rom) - 80.30785563).abs() < 1e-8
        );

        assert!((lang_rep.size_pct(SymbolLang::C, MemoryRegion::Ram) - 98.20359281).abs() < 1e-8);
        assert!((lang_rep.size_pct(SymbolLang::Cpp, MemoryRegion::Ram) - 0_f64).abs() < 1e-8);
        assert!(
            (lang_rep.size_pct(SymbolLang::Rust, MemoryRegion::Ram) - 1.796407186).abs() < 1e-8
        );
    }
    ...
    fn report_lang_c_app_c_lib_rust_lib() {
        ...
        assert!((lang_rep.size_pct(SymbolLang::Cpp, MemoryRegion::Both) - 0_f64).abs() < 1e-8);
        assert!(
            (lang_rep.size_pct(SymbolLang::Rust, MemoryRegion::Both) - 57.60151086).abs() < 1e-8
        );

        assert!((lang_rep.size_pct(SymbolLang::C, MemoryRegion::Rom) - 21.48417229).abs() < 1e-8);
        assert!((lang_rep.size_pct(SymbolLang::Cpp, MemoryRegion::Rom) - 0_f64).abs() < 1e-8);
        assert!(
            (lang_rep.size_pct(SymbolLang::Rust, MemoryRegion::Rom) - 78.51582771).abs() < 1e-8
        );

        assert!((lang_rep.size_pct(SymbolLang::C, MemoryRegion::Ram) - 98.33448994).abs() < 1e-8);
        assert!((lang_rep.size_pct(SymbolLang::Cpp, MemoryRegion::Ram) - 0_f64).abs() < 1e-8);
        assert!(
            (lang_rep.size_pct(SymbolLang::Rust, MemoryRegion::Ram) - 1.665510062).abs() < 1e-8
        );
    }
    ...
    fn report_syms_iter_c_app_rust_lib() {
        ...
        let syms_rep = at
            .report_syms(vec![SymbolLang::Any], MemoryRegion::Rom, Some(2))
            .unwrap();
        ...
        let syms_rep = at
            .report_syms(vec![SymbolLang::Rust], MemoryRegion::Ram, Some(2))
            .unwrap();
        ...
        let syms_rep = at
            .report_syms(vec![SymbolLang::C], MemoryRegion::Both, Some(3))
            .unwrap();
    ...
    fn report_syms_iter_c_app_c_lib_rust_lib() {
        ...
        let syms_rep = at
            .report_syms(vec![SymbolLang::Rust], MemoryRegion::Rom, Some(4))
            .unwrap();
        ...
        let syms_rep = at
            .report_syms(vec![SymbolLang::Any], MemoryRegion::Ram, None)
            .unwrap();
