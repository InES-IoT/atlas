...
pub struct Atlas {
    ...
    lock: PathBuf,
    lib: PathBuf,
    syms: Vec<sym::SymbolGuess>,
    fails: Vec<(String,String)>,
}

impl Atlas {
    pub fn new<N,E,O,L>(nm: N, elf: E, lock: O, lib: L) -> io::Result<Self>
    where
        N: AsRef<Path>,
        E: AsRef<Path>,
        O: AsRef<Path>,
        L: AsRef<Path>,
    {
        ...
        let lock = curr.join(lock.as_ref()).canonicalize()?;
    ...
    pub fn analyze(&mut self) -> io::Result<()> {
        ...
        gsr.from_lock(&self.lock).unwrap();
        gsr.from_lib(&self.nm, &self.lib).unwrap();
...
mod tests {
    ...
    #[test]
    fn new_str() {
        let at = Atlas::new(&*NM_PATH, file!(), file!(), file!());
        assert!(at.is_ok());
    }

    #[test]
    fn new_string() {
        let at = Atlas::new(&*NM_PATH, String::from(file!()), String::from(file!()), String::from(file!()));
        assert!(at.is_ok());
    }

    #[test]
    fn new_pathbuf() {
        let at = Atlas::new(&*NM_PATH, PathBuf::from(file!()), PathBuf::from(file!()), PathBuf::from(file!()));
        assert!(at.is_ok());
    }

    #[test]
    fn new_path() {
        let at = Atlas::new(&*NM_PATH, Path::new(file!()), Path::new(file!()), Path::new(file!()));
        assert!(at.is_ok());
    }

    #[test]
    fn new_mixed() {
        let at = Atlas::new(&*NM_PATH, PathBuf::from(file!()), Path::new(file!()), file!());
        assert!(at.is_ok());
    }

    #[test]
    fn new_canonicalize() {
        let at = Atlas::new(&*NM_PATH, "/etc/hostname", "./aux/../src/../Cargo.toml", "src/lib.rs");
        assert!(at.is_ok());
    }

    #[test]
    fn illegal_path() {
        let err = Atlas::new(&*NM_PATH, "kljsdflkjsdf", "ljksdflkjsdflsj", "lkjsdflkjsdf").unwrap_err();
        assert_eq!(err.kind(), ErrorKind::NotFound);
    }

    #[test]
    fn permission_denied() {
        let err = Atlas::new(&*NM_PATH, file!(), file!(), "/etc/shadow").unwrap_err();
        assert_eq!(err.kind(), ErrorKind::PermissionDenied);
    }

    #[test]
    fn nm_wrong_file_type() {
        let mut at = Atlas::new(&*NM_PATH, "../README.md", "aux/Cargo.lock", "aux/libsecprint.a").unwrap();
    ...
    #[test]
    fn analyze() {
        let mut at = Atlas::new(&*NM_PATH, "aux/rust_minimal_node.elf", "aux/Cargo.lock", "aux/libsecprint.a").unwrap();
