...
mod langdetector_tests {
    ...
    fn add_rust_lib() {
        ...
        assert_eq!(detector.lib_syms.len(), 2493);
    }
    ...
    fn add_rust_lib_bad_nm_path() {
        ...
        let lib = lib.canonicalize().unwrap();
        let mut detector = LangDetector::new();
        assert!(detector.add_rust_lib("/bad/path", lib).is_err());
    }

    #[test]
    fn add_rust_lib_permission_denied() {
        let mut detector = LangDetector::new();
        assert!(detector.add_rust_lib(&*NM_PATH, "/etc/shadow").is_err());
    }
